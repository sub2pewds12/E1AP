...
func (msg *DeactivateTrace) Encode(w io.Writer) error {
	...
	return encodeMessage(w, E1apPduInitiatingMessage, ProcedureCode{Value: ProcedureCodeDeactivateTrace}, Criticality{Value: CriticalityReject}, ies)
}

// Decode implements the aper.AperUnmarshaller interface for DeactivateTrace.
func (msg *DeactivateTrace) Decode(buf []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("decode DeactivateTrace failed: %w", err)
		}
	}()
