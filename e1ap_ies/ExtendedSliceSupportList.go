...
func (s *ExtendedSliceSupportList) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 1, Ub: MaxnoofExtSliceItems}, false); err != nil {
		return fmt.Errorf("WriteSequenceOf for ExtendedSliceSupportList failed: %w", err)
	}
	return nil
}
