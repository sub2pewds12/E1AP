...
// DRBSetupModListEUTRAN is a generated LIST type.
type DRBSetupModListEUTRAN struct {
	Value []DRBSetupModItemEUTRAN
}

func (s *DRBSetupModListEUTRAN) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 1, Ub: MaxnoofDRBs}, false); err != nil {
		return fmt.Errorf("writeSequenceOf for DRBSetupModListEUTRAN failed: %w", err)
	}
	return nil
}

func (s *DRBSetupModListEUTRAN) Decode(r *aper.AperReader) (err error) {

	// 1. Create a decoder function for the item type.
	decoder := func(r *aper.AperReader) (*DRBSetupModItemEUTRAN, error) {
	...
	var decodedItems []DRBSetupModItemEUTRAN // <--- FIX: Removed the '*'
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 1, Ub: MaxnoofDRBs}, false); err != nil {
		return fmt.Errorf("readSequenceOf for DRBSetupModListEUTRAN failed: %w", err)
	}
