...
// Encode implements the aper.AperMarshaller interface for GNBCUCPConfigurationUpdateFailure.
func (msg *GNBCUCPConfigurationUpdateFailure) Encode(w io.Writer) error {
	ies, err := msg.toIes()
	if err != nil {
		return fmt.Errorf("could not convert GNBCUCPConfigurationUpdateFailure to IEs: %w", err)
	}

	return encodeMessage(w, E1apPduUnsuccessfulOutcome, ProcedureCode{Value: ProcedureCodeGNBCUCPConfigurationUpdate}, Criticality{Value: CriticalityIgnore}, ies)
}

// Decode implements the aper.AperUnmarshaller interface for GNBCUCPConfigurationUpdateFailure.
func (msg *GNBCUCPConfigurationUpdateFailure) Decode(buf []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("GNBCUCPConfigurationUpdateFailure: %w", err)
		}
	}()
...
func (decoder *GNBCUCPConfigurationUpdateFailureDecoder) decodeIE(r *aper.AperReader) (msgIe *E1APMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
