...
// QOSFlowsToBeForwardedList is a generated LIST type.
type QOSFlowsToBeForwardedList struct {
	Value []QOSFlowsToBeForwardedItem
}

func (s *QOSFlowsToBeForwardedList) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 1, Ub: MaxnoofQoSFlows}, false); err != nil {
		return fmt.Errorf("writeSequenceOf for QOSFlowsToBeForwardedList failed: %w", err)
	}
	return nil
}

func (s *QOSFlowsToBeForwardedList) Decode(r *aper.AperReader) (err error) {

	// 1. Create a decoder function for the item type.
	decoder := func(r *aper.AperReader) (*QOSFlowsToBeForwardedItem, error) {
		item := new(QOSFlowsToBeForwardedItem)
	...
	var decodedItems []QOSFlowsToBeForwardedItem // <--- FIX: Removed the '*'
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 1, Ub: MaxnoofQoSFlows}, false); err != nil {
		return fmt.Errorf("readSequenceOf for QOSFlowsToBeForwardedList failed: %w", err)
	}
