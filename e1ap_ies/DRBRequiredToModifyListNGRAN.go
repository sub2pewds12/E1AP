...
func (s *DRBRequiredToModifyListNGRAN) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 1, Ub: MaxnoofDRBs}, false); err != nil {
		return fmt.Errorf("WriteSequenceOf for DRBRequiredToModifyListNGRAN failed: %w", err)
	}
	return nil
}

func (s *DRBRequiredToModifyListNGRAN) Decode(r *aper.AperReader) (err error) {

	// 1. Create a decoder function for the item type.
	decoder := func(r *aper.AperReader) (*DRBRequiredToModifyItemNGRAN, error) {
	...
	var decodedItems []DRBRequiredToModifyItemNGRAN // <--- FIX: Removed the '*'
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 1, Ub: MaxnoofDRBs}, false); err != nil {
		return fmt.Errorf("ReadSequenceOf for DRBRequiredToModifyListNGRAN failed: %w", err)
	}
