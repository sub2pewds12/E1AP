...
func (msg *BearerContextModificationResponse) Encode(w io.Writer) error {
	...
	return EncodeSuccessfulOutcome(w, ProcedureCodeBearerContextModification, Criticality{Value: CriticalityIgnore}, ies)
}

// Decode implements the aper.AperUnmarshaller interface for BearerContextModificationResponse.
func (msg *BearerContextModificationResponse) Decode(buf []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("BearerContextModificationResponse: %w", err)
		}
	}()
...
func (decoder *BearerContextModificationResponseDecoder) decodeIE(r *aper.AperReader) (msgIe *E1APMessageIE, err error) {
	...
	case ProtocolIEIDSystemBearerContextModificationResponse:
		s.SystemBearerContextModificationResponse = new(SystemBearerContextModificationResponse)
		if err = s.SystemBearerContextModificationResponse.Decode(r); err != nil {
			return fmt.Errorf("Decode SystemBearerContextModificationResponse failed: %w", err)
		}
	default:
