...
func (s *NGRANQOSSupportList) Encode(w *per.Encoder) (err error) {

	c := per.SizeConstraints{Extensible: false, Min: int64Ptr(1), Max: int64Ptr(MaxnoofNGRANQOSParameters)}
...
func (s *NGRANQOSSupportList) Decode(r *per.Decoder) (err error) {
	...
	s.Value = make([]NGRANQOSSupportItem, length)
