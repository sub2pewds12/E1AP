...
func (msg *GNBCUUPConfigurationUpdateAcknowledge) EncodeWithEncoder(e *per.Encoder) (err error) {
	ies, err := msg.toIes()
	if err != nil {
		return err
	}
...
// Decode implements the MessageUnmarshaller interface for GNBCUUPConfigurationUpdateAcknowledge.
func (msg *GNBCUUPConfigurationUpdateAcknowledge) Decode(data []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	r := per.NewDecoder(data, per.APER)
	return msg.DecodeFromDecoder(r)
}

func (msg *GNBCUUPConfigurationUpdateAcknowledge) DecodeFromDecoder(r *per.Decoder) (diagList []CriticalityDiagnosticsIEItem, err error) {

	defer func() {
		if err != nil {
			err = fmt.Errorf("decode GNBCUUPConfigurationUpdateAcknowledge failed: %w", err)
		}
	}()
...
func (decoder *GNBCUUPConfigurationUpdateAcknowledgeDecoder) decodeIE(r *per.Decoder) (msgIe *E1APMessageIE, err error) {
