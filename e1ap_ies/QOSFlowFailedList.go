...
// QOSFlowFailedList is a generated LIST type.
type QOSFlowFailedList struct {
	Value []QOSFlowFailedItem
}

func (s *QOSFlowFailedList) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 1, Ub: MaxnoofQoSFlows}, false); err != nil {
		return fmt.Errorf("WriteSequenceOf for QOSFlowFailedList failed: %w", err)
	}
	return nil
}

func (s *QOSFlowFailedList) Decode(r *aper.AperReader) (err error) {

	// 1. Create a decoder function for the item type.
	decoder := func(r *aper.AperReader) (*QOSFlowFailedItem, error) {
		item := new(QOSFlowFailedItem)
	...
	var decodedItems []QOSFlowFailedItem // <--- FIX: Removed the '*'
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 1, Ub: MaxnoofQoSFlows}, false); err != nil {
		return fmt.Errorf("ReadSequenceOf for QOSFlowFailedList failed: %w", err)
	}
