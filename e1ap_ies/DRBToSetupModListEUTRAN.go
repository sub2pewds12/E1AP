...
func (s *DRBToSetupModListEUTRAN) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 1, Ub: MaxnoofDRBs}, false); err != nil {
		return fmt.Errorf("writeSequenceOf for DRBToSetupModListEUTRAN failed: %w", err)
	}
	return nil
}

func (s *DRBToSetupModListEUTRAN) Decode(r *aper.AperReader) (err error) {

	// 1. Create a decoder function for the item type.
	decoder := func(r *aper.AperReader) (*DRBToSetupModItemEUTRAN, error) {
	...
	var decodedItems []DRBToSetupModItemEUTRAN // <--- FIX: Removed the '*'
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 1, Ub: MaxnoofDRBs}, false); err != nil {
		return fmt.Errorf("readSequenceOf for DRBToSetupModListEUTRAN failed: %w", err)
	}
