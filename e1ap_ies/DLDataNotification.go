...
func (msg *DLDataNotification) toIes() ([]E1APMessageIE, error) {
	...
	if msg.PDUSessionToNotifyList != nil {
		...
		ies = append(ies, E1APMessageIE{
			...
			Criticality: Criticality{Value: CriticalityIgnore},
			Value:       msg.PDUSessionToNotifyList,
		})
	}
	return ies, nil
}

func (msg *DLDataNotification) EncodeWithEncoder(e *per.Encoder) (err error) {
	ies, err := msg.toIes()
	if err != nil {
		return err
	}
...
// Decode implements the MessageUnmarshaller interface for DLDataNotification.
func (msg *DLDataNotification) Decode(data []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	r := per.NewDecoder(data, per.APER)
	return msg.DecodeFromDecoder(r)
}

func (msg *DLDataNotification) DecodeFromDecoder(r *per.Decoder) (diagList []CriticalityDiagnosticsIEItem, err error) {
...
func (decoder *DLDataNotificationDecoder) decodeIE(r *per.Decoder) (msgIe *E1APMessageIE, err error) {
	...
	case ProtocolIEIDPPI:
		...
		{
			val, err := ieR.DecodeInteger(per.ConstrainedExtensible(0, 7))
			if err != nil {
	...
	case ProtocolIEIDPDUSessionToNotifyList:
		...
		{
			itemDecoder := func(r *per.Decoder) (*PDUSessionToNotifyItem, error) {
			...
			for i := int64(0); i < length; i++ {
				...
				msg.PDUSessionToNotifyList.Value = append(msg.PDUSessionToNotifyList.Value, *item)
			}
		}
	default:
