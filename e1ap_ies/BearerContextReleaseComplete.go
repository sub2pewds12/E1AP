...
func (msg *BearerContextReleaseComplete) EncodeWithEncoder(e *per.Encoder) (err error) {
	ies, err := msg.toIes()
	if err != nil {
		return err
	}
...
// Decode implements the MessageUnmarshaller interface for BearerContextReleaseComplete.
func (msg *BearerContextReleaseComplete) Decode(data []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	r := per.NewDecoder(data, per.APER)
	return msg.DecodeFromDecoder(r)
}

func (msg *BearerContextReleaseComplete) DecodeFromDecoder(r *per.Decoder) (diagList []CriticalityDiagnosticsIEItem, err error) {
...
func (decoder *BearerContextReleaseCompleteDecoder) decodeIE(r *per.Decoder) (msgIe *E1APMessageIE, err error) {
	...
	case ProtocolIEIDRetainabilityMeasurementsInfo:
		...
		{
			itemDecoder := func(r *per.Decoder) (*DRBRemovedItem, error) {
			...
			for i := int64(0); i < length; i++ {
				...
				msg.RetainabilityMeasurementsInfo.Value = append(msg.RetainabilityMeasurementsInfo.Value, *item)
			}
		}
	default:
