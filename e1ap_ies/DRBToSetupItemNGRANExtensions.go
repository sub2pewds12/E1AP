...
func (s *DRBToSetupItemNGRANExtensions) Encode(w *per.Encoder) error {
...
func (s *DRBToSetupItemNGRANExtensions) Decode(r *per.Decoder) error {
	c := per.SizeConstraints{Extensible: false, Min: int64Ptr(1), Max: int64Ptr(MaxProtocolExtensions)}
	...
	for _, ext := range extensions {
		...
		case ProtocolIEIDQoSFlowsDRBRemapping:
			...
			if err := s.QoSFlowsDRBRemapping.Decode(per.NewDecoder(ext.ValueBytes, per.APER)); err != nil {
