...
// Encode implements the aper.AperMarshaller interface for GNBCUUPConfigurationUpdateFailure.
func (msg *GNBCUUPConfigurationUpdateFailure) Encode(w io.Writer) error {
	ies, err := msg.toIes()
	if err != nil {
		return fmt.Errorf("could not convert GNBCUUPConfigurationUpdateFailure to IEs: %w", err)
	}

	return encodeMessage(w, E1apPduUnsuccessfulOutcome, ProcedureCode{Value: ProcedureCodeGNBCUUPConfigurationUpdate}, Criticality{Value: CriticalityIgnore}, ies)
}

// Decode implements the aper.AperUnmarshaller interface for GNBCUUPConfigurationUpdateFailure.
func (msg *GNBCUUPConfigurationUpdateFailure) Decode(buf []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	defer func() {
		if err != nil {
