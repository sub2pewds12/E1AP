...
type PDUSessionToNotifyItem struct {
	...
	IEExtensions *PDUSessionToNotifyItemExtensions
}

// Encode implements the aper.AperMarshaller interface.
func (s *PDUSessionToNotifyItem) Encode(w *per.Encoder) (err error) {

	c := per.SequenceConstraints{
		Extensible: true,
		RootComponents: []per.ComponentInfo{
			per.ComponentInfo{Name: "pDU-Session-ID", Optional: false},
			per.ComponentInfo{Name: "qoS-Flow-List", Optional: false},
...
func (s *PDUSessionToNotifyItem) Decode(r *per.Decoder) (err error) {
	...
	if seqDecoder.IsComponentPresent(2) {
		s.IEExtensions = new(PDUSessionToNotifyItemExtensions)
