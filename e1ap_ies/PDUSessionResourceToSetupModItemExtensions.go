...
func (s *PDUSessionResourceToSetupModItemExtensions) Encode(w *per.Encoder) error {
...
func (s *PDUSessionResourceToSetupModItemExtensions) Decode(r *per.Decoder) error {
	c := per.SizeConstraints{Extensible: false, Min: int64Ptr(1), Max: int64Ptr(MaxProtocolExtensions)}
	...
	for _, ext := range extensions {
		...
		case ProtocolIEIDNetworkInstance:
			...
			if err := s.NetworkInstance.Decode(per.NewDecoder(ext.ValueBytes, per.APER)); err != nil {
