...
func (msg *TraceStart) EncodeWithEncoder(e *per.Encoder) (err error) {
	ies, err := msg.toIes()
	if err != nil {
		return err
	}
...
// Decode implements the MessageUnmarshaller interface for TraceStart.
func (msg *TraceStart) Decode(data []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	r := per.NewDecoder(data, per.APER)
	return msg.DecodeFromDecoder(r)
}

func (msg *TraceStart) DecodeFromDecoder(r *per.Decoder) (diagList []CriticalityDiagnosticsIEItem, err error) {

	defer func() {
		if err != nil {
			err = fmt.Errorf("decode TraceStart failed: %w", err)
		}
	}()
...
func (decoder *TraceStartDecoder) decodeIE(r *per.Decoder) (msgIe *E1APMessageIE, err error) {
