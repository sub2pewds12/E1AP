...
// SliceSupportList is a generated LIST type.
type SliceSupportList struct {
	Value []SliceSupportItem
}

func (s *SliceSupportList) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 1, Ub: MaxnoofSliceItems}, false); err != nil {
		return fmt.Errorf("writeSequenceOf for SliceSupportList failed: %w", err)
	}
	return nil
}

func (s *SliceSupportList) Decode(r *aper.AperReader) (err error) {
	...
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 1, Ub: MaxnoofSliceItems}, false); err != nil {
		return fmt.Errorf("readSequenceOf for SliceSupportList failed: %w", err)
	}
