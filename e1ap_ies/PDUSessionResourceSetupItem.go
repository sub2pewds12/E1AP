...
func (s *PDUSessionResourceSetupItem) Encode(w *aper.AperWriter) (err error) {
	...
	if s.NGDLUPUnchanged != nil {
		if err = s.NGDLUPUnchanged.Encode(w); err != nil {
...
func (s *PDUSessionResourceSetupItem) Decode(r *aper.AperReader) (err error) {
	...
	if len(optionalityBitmap) > 0 && optionalityBitmap[0]&(1<<5) > 0 {
...

		{
			...
			s.NGDLUPUnchanged.Value = aper.Enumerated(val)
		}
	}
	...
	if isExtensible {
		return fmt.Errorf("Extensions not yet implemented for PDUSessionResourceSetupItem")
	}
	return nil
}
