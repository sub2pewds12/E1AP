...
func (s *ExtendedNRCGISupportList) Encode(w *per.Encoder) (err error) {

	c := per.SizeConstraints{Extensible: false, Min: int64Ptr(1), Max: int64Ptr(MaxnoofExtNRCGI)}
...
func (s *ExtendedNRCGISupportList) Decode(r *per.Decoder) (err error) {
	...
	s.Value = make([]ExtendedNRCGISupportItem, length)
