...
func (msg *BearerContextModificationRequired) Encode(w io.Writer) error {
	...
	return encodeMessage(w, E1apPduInitiatingMessage, ProcedureCodeBearerContextModificationRequired, Criticality{Value: CriticalityReject}, ies)
}

// Decode implements the aper.AperUnmarshaller interface for BearerContextModificationRequired.
func (msg *BearerContextModificationRequired) Decode(buf []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("BearerContextModificationRequired: %w", err)
		}
	}()
	...
	if _, ok := decoder.list[ProtocolIEIDSystemBearerContextModificationRequired]; !ok {
		err = fmt.Errorf("mandatory field SystemBearerContextModificationRequired is missing")
		diagList = append(diagList, CriticalityDiagnosticsIEItem{
			IECriticality: Criticality{Value: CriticalityReject},
			IEID:          ProtocolIEIDSystemBearerContextModificationRequired,
			TypeOfError:   TypeOfError{Value: TypeOfErrorMissing},
		})
	}
	if err != nil {
