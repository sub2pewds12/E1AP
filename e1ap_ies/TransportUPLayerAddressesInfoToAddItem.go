...
type TransportUPLayerAddressesInfoToAddItem struct {
	...
	GTPTransportLayerAddressesToAdd *GTPTLAs
	IEExtensions                    *TransportUPLayerAddressesInfoToAddItemExtensions
}

// Encode implements the aper.AperMarshaller interface.
func (s *TransportUPLayerAddressesInfoToAddItem) Encode(w *per.Encoder) (err error) {

	c := per.SequenceConstraints{
		Extensible: true,
		RootComponents: []per.ComponentInfo{
			per.ComponentInfo{Name: "iP-SecTransportLayerAddress", Optional: false},
			per.ComponentInfo{Name: "gTPTransportLayerAddressesToAdd", Optional: true},
...
func (s *TransportUPLayerAddressesInfoToAddItem) Decode(r *per.Decoder) (err error) {
	...
	if seqDecoder.IsComponentPresent(2) {
		s.IEExtensions = new(TransportUPLayerAddressesInfoToAddItemExtensions)
