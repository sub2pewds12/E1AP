...
func (msg *GNBCUUPStatusIndication) EncodeWithEncoder(e *per.Encoder) (err error) {
	ies, err := msg.toIes()
	if err != nil {
		return err
	}
...
// Decode implements the MessageUnmarshaller interface for GNBCUUPStatusIndication.
func (msg *GNBCUUPStatusIndication) Decode(data []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	r := per.NewDecoder(data, per.APER)
	return msg.DecodeFromDecoder(r)
}

func (msg *GNBCUUPStatusIndication) DecodeFromDecoder(r *per.Decoder) (diagList []CriticalityDiagnosticsIEItem, err error) {

	defer func() {
		if err != nil {
			err = fmt.Errorf("decode GNBCUUPStatusIndication failed: %w", err)
		}
	}()
...
func (decoder *GNBCUUPStatusIndicationDecoder) decodeIE(r *per.Decoder) (msgIe *E1APMessageIE, err error) {
	...
	case ProtocolIEIDGNBCUUPOverloadInformation:
		...
		{
			c := per.EnumeratedConstraints{Extensible: false, RootValues: make([]int64, 2)}
			val, err := ieR.DecodeEnumerated(c)
			if err != nil {
				return nil, fmt.Errorf("decode GNBCUUPOverloadInformation failed: %w", err)
			}
			msg.GNBCUUPOverloadInformation.Value = val
		}
	default:
