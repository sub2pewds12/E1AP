...
func (s *AlternativeQoSParaSetList) Encode(w *per.Encoder) (err error) {

	c := per.SizeConstraints{Extensible: false, Min: int64Ptr(1), Max: int64Ptr(MaxnoofQoSParaSets)}
...
func (s *AlternativeQoSParaSetList) Decode(r *per.Decoder) (err error) {
	...
	s.Value = make([]AlternativeQoSParaSetItem, length)
