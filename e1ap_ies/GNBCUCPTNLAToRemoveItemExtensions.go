...
func (s *GNBCUCPTNLAToRemoveItemExtensions) Encode(w *per.Encoder) error {
...
func (s *GNBCUCPTNLAToRemoveItemExtensions) Decode(r *per.Decoder) error {
	c := per.SizeConstraints{Extensible: false, Min: int64Ptr(1), Max: int64Ptr(MaxProtocolExtensions)}
	...
	for _, ext := range extensions {
		...
		case ProtocolIEIDTNLAssociationTransportLayerAddressgNBCUUP:
			...
			if err := s.TNLAssociationTransportLayerAddressgNBCUUP.Decode(per.NewDecoder(ext.ValueBytes, per.APER)); err != nil {
