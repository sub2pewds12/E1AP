...
type DRBStatusItem struct {
	...
	IEExtensions *DRBStatusItemExtensions
}

// Encode implements the aper.AperMarshaller interface.
func (s *DRBStatusItem) Encode(w *per.Encoder) (err error) {

	c := per.SequenceConstraints{
		Extensible: true,
		RootComponents: []per.ComponentInfo{
			per.ComponentInfo{Name: "dRB-ID", Optional: false},
			per.ComponentInfo{Name: "pDCP-DL-Count", Optional: true},
			per.ComponentInfo{Name: "pDCP-UL-Count", Optional: true},
...
func (s *DRBStatusItem) Decode(r *per.Decoder) (err error) {
	...
	if seqDecoder.IsComponentPresent(3) {
		s.IEExtensions = new(DRBStatusItemExtensions)
