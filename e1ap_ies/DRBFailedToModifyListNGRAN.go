...
func (s *DRBFailedToModifyListNGRAN) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 1, Ub: MaxnoofDRBs}, false); err != nil {
		return fmt.Errorf("WriteSequenceOf for DRBFailedToModifyListNGRAN failed: %w", err)
	}
	return nil
}

func (s *DRBFailedToModifyListNGRAN) Decode(r *aper.AperReader) (err error) {

	// 1. Create a decoder function for the item type.
	decoder := func(r *aper.AperReader) (*DRBFailedToModifyItemNGRAN, error) {
	...
	var decodedItems []DRBFailedToModifyItemNGRAN // <--- FIX: Removed the '*'
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 1, Ub: MaxnoofDRBs}, false); err != nil {
		return fmt.Errorf("ReadSequenceOf for DRBFailedToModifyListNGRAN failed: %w", err)
	}
