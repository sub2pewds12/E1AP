...
// DRBSetupListNGRAN is a generated LIST type.
type DRBSetupListNGRAN struct {
	Value []DRBSetupItemNGRAN
}

func (s *DRBSetupListNGRAN) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 1, Ub: MaxnoofDRBs}, false); err != nil {
		return fmt.Errorf("writeSequenceOf for DRBSetupListNGRAN failed: %w", err)
	}
	return nil
}

func (s *DRBSetupListNGRAN) Decode(r *aper.AperReader) (err error) {

	// 1. Create a decoder function for the item type.
	decoder := func(r *aper.AperReader) (*DRBSetupItemNGRAN, error) {
		item := new(DRBSetupItemNGRAN)
	...
	var decodedItems []DRBSetupItemNGRAN // <--- FIX: Removed the '*'
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 1, Ub: MaxnoofDRBs}, false); err != nil {
		return fmt.Errorf("readSequenceOf for DRBSetupListNGRAN failed: %w", err)
	}
