...
func (msg *BearerContextModificationConfirm) EncodeWithEncoder(e *per.Encoder) (err error) {
	ies, err := msg.toIes()
	if err != nil {
		return err
	}
...
// Decode implements the MessageUnmarshaller interface for BearerContextModificationConfirm.
func (msg *BearerContextModificationConfirm) Decode(data []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	r := per.NewDecoder(data, per.APER)
	return msg.DecodeFromDecoder(r)
}

func (msg *BearerContextModificationConfirm) DecodeFromDecoder(r *per.Decoder) (diagList []CriticalityDiagnosticsIEItem, err error) {
...
func (decoder *BearerContextModificationConfirmDecoder) decodeIE(r *per.Decoder) (msgIe *E1APMessageIE, err error) {
