...
func (s *DataForwardingtoEUTRANInformationList) Encode(w *per.Encoder) (err error) {

	c := per.SizeConstraints{Extensible: false, Min: int64Ptr(1), Max: int64Ptr(MaxnoofDataForwardingTunneltoEUTRAN)}
...
func (s *DataForwardingtoEUTRANInformationList) Decode(r *per.Decoder) (err error) {
	...
	s.Value = make([]DataForwardingtoEUTRANInformationListItem, length)
