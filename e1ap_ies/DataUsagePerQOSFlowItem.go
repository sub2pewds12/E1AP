...
func (s *DataUsagePerQOSFlowItem) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 1, Ub: Maxnooftimeperiods}, true); err != nil {
		return fmt.Errorf("writeSequenceOf for DataUsagePerQOSFlowItem failed: %w", err)
	}
	return nil
}

func (s *DataUsagePerQOSFlowItem) Decode(r *aper.AperReader) (err error) {
	...
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 1, Ub: Maxnooftimeperiods}, true); err != nil {
		return fmt.Errorf("readSequenceOf for DataUsagePerQOSFlowItem failed: %w", err)
	}
