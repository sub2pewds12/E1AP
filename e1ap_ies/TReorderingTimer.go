...
func (s *TReorderingTimer) Decode(r *aper.AperReader) (err error) {
	...
	{
		var val uint64
		if val, err = r.ReadEnumerate(aper.Constraint{Lb: 0, Ub: 35}, true); err != nil {
			return fmt.Errorf("Decode TReordering failed: %w", err)
		}
		s.TReordering.Value = aper.Enumerated(val)
	}
	...
	if isExtensible {
		return fmt.Errorf("Extensions not yet implemented for TReorderingTimer")
	}
	return nil
}
