...
func (msg *E1ReleaseRequest) Encode(w io.Writer) error {
	...
	return encodeMessage(w, E1apPduInitiatingMessage, ProcedureCodeE1Release, Criticality{Value: CriticalityReject}, ies)
}

// Decode implements the aper.AperUnmarshaller interface for E1ReleaseRequest.
func (msg *E1ReleaseRequest) Decode(buf []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("E1ReleaseRequest: %w", err)
		}
	}()
