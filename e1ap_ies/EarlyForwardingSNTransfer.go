...
type EarlyForwardingSNTransfer struct {
	...
	DRBsSubjectToEarlyForwardingList DRBsSubjectToEarlyForwardingList
}
...
func (msg *EarlyForwardingSNTransfer) toIes() ([]E1APMessageIE, error) {
	...
	ies = append(ies, E1APMessageIE{
		...
		Criticality: Criticality{Value: CriticalityReject},
		Value:       &msg.DRBsSubjectToEarlyForwardingList,
	})
	return ies, nil
}

func (msg *EarlyForwardingSNTransfer) EncodeWithEncoder(e *per.Encoder) (err error) {
	ies, err := msg.toIes()
	if err != nil {
		return err
	}
...
// Decode implements the MessageUnmarshaller interface for EarlyForwardingSNTransfer.
func (msg *EarlyForwardingSNTransfer) Decode(data []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	r := per.NewDecoder(data, per.APER)
	return msg.DecodeFromDecoder(r)
}

func (msg *EarlyForwardingSNTransfer) DecodeFromDecoder(r *per.Decoder) (diagList []CriticalityDiagnosticsIEItem, err error) {
...
func (decoder *EarlyForwardingSNTransferDecoder) decodeIE(r *per.Decoder) (msgIe *E1APMessageIE, err error) {
	...
	case ProtocolIEIDDRBsSubjectToEarlyForwardingList:
		...
		{
			itemDecoder := func(r *per.Decoder) (*DRBsSubjectToEarlyForwardingItem, error) {
			...
			for i := int64(0); i < length; i++ {
				...
				msg.DRBsSubjectToEarlyForwardingList.Value = append(msg.DRBsSubjectToEarlyForwardingList.Value, *item)
			}
		}
	default:
