...
func (s *DRBRemovedItem) Encode(w *per.Encoder) (err error) {

	c := per.SizeConstraints{Extensible: true, Min: int64Ptr(1), Max: int64Ptr(MaxnoofQoSFlows)}
...
func (s *DRBRemovedItem) Decode(r *per.Decoder) (err error) {
	...
	s.Value = make([]QOSFlowRemovedItem, length)
