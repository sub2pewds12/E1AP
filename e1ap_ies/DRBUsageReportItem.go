...
type DRBUsageReportItem struct {
	StartTimeStamp DRBUsageReportItemStartTimeStamp
	EndTimeStamp   DRBUsageReportItemEndTimeStamp
	UsageCountUL   DRBUsageReportItemUsageCountUL
	UsageCountDL   DRBUsageReportItemUsageCountDL
	IEExtensions   *DRBUsageReportItemExtensions
}

// Encode implements the aper.AperMarshaller interface.
func (s *DRBUsageReportItem) Encode(w *per.Encoder) (err error) {
...
func (s *DRBUsageReportItem) Decode(r *per.Decoder) (err error) {
	...
	if seqDecoder.IsComponentPresent(4) {
		s.IEExtensions = new(DRBUsageReportItemExtensions)
