...
func (s *DRBRemovedItem) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 5, Ub: 5}, true); err != nil {
		return fmt.Errorf("WriteSequenceOf for DRBRemovedItem failed: %w", err)
	}
	return nil
}
...
func (s *DRBRemovedItem) Decode(r *aper.AperReader) (err error) {
	...
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 5, Ub: 5}, true); err != nil {
