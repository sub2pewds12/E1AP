...
func (msg *BearerContextModificationRequired) EncodeWithEncoder(e *per.Encoder) (err error) {
	ies, err := msg.toIes()
	if err != nil {
		return err
	}
...
// Decode implements the MessageUnmarshaller interface for BearerContextModificationRequired.
func (msg *BearerContextModificationRequired) Decode(data []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	r := per.NewDecoder(data, per.APER)
	return msg.DecodeFromDecoder(r)
}

func (msg *BearerContextModificationRequired) DecodeFromDecoder(r *per.Decoder) (diagList []CriticalityDiagnosticsIEItem, err error) {
...
func (decoder *BearerContextModificationRequiredDecoder) decodeIE(r *per.Decoder) (msgIe *E1APMessageIE, err error) {
