...
func (s *AlternativeQoSParaSetItem) Encode(w *aper.AperWriter) (err error) {
	...
	if err = w.WriteInteger(int64(s.AlternativeQoSParameterIndex), &aper.Constraint{Lb: 1, Ub: 8}, true); err != nil {
		return fmt.Errorf("Encode AlternativeQoSParameterIndex failed: %w", err)
	}
	if s.GuaranteedFlowBitRateDL != nil {
		if err = w.WriteInteger(int64((*s.GuaranteedFlowBitRateDL)), &aper.Constraint{Lb: 0, Ub: 4000000000000}, true); err != nil {
			return fmt.Errorf("Encode GuaranteedFlowBitRateDL failed: %w", err)
		}
	}
	if s.GuaranteedFlowBitRateUL != nil {
		if err = w.WriteInteger(int64((*s.GuaranteedFlowBitRateUL)), &aper.Constraint{Lb: 0, Ub: 4000000000000}, true); err != nil {
...
func (s *AlternativeQoSParaSetItem) Decode(r *aper.AperReader) (err error) {
...

	{
		...
		s.AlternativeQoSParameterIndex = AlternativeQoSParaSetItemAlternativeQoSParameterIndex(val)
	}
	...
	if len(optionalityBitmap) > 0 && optionalityBitmap[0]&(1<<5) > 0 {

		{
			...
			tmp := PacketDelayBudget(val)
			s.PacketDelayBudget = &tmp
		}
