...
// toIes transforms the DeactivateTrace struct into a slice of E1APMessageIEs.
func (msg *DeactivateTrace) toIes() ([]E1APMessageIE, error) {
...
// Encode implements the aper.AperMarshaller interface for DeactivateTrace.
func (msg *DeactivateTrace) Encode(w io.Writer) error {
	ies, err := msg.toIes()
	if err != nil {
		return fmt.Errorf("could not convert DeactivateTrace to IEs: %w", err)
	}

	return encodeMessage(w, E1apPduInitiatingMessage, ProcedureCodeDeactivateTrace, Criticality{Value: CriticalityReject}, ies)
}

// Decode implements the aper.AperUnmarshaller interface for DeactivateTrace.
func (msg *DeactivateTrace) Decode(buf []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("DeactivateTrace: %w", err)
		}
	}()
	...
	if _, ok := decoder.list[ProtocolIEIDTraceID]; !ok {
		err = fmt.Errorf("mandatory field TraceID is missing")
		diagList = append(diagList, CriticalityDiagnosticsIEItem{
			IECriticality: Criticality{Value: CriticalityReject},
			IEID:          ProtocolIEIDTraceID,
			TypeOfError:   TypeOfError{Value: TypeOfErrorMissing},
		})
	}
	if err != nil {
...
func (decoder *DeactivateTraceDecoder) decodeIE(r *aper.AperReader) (msgIe *E1APMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
