...
// DRBToRemoveListEUTRAN is a generated LIST type.
type DRBToRemoveListEUTRAN struct {
	Value []DRBToRemoveItemEUTRAN
}

func (s *DRBToRemoveListEUTRAN) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 0, Ub: 0}, false); err != nil {
		return fmt.Errorf("WriteSequenceOf for DRBToRemoveListEUTRAN failed: %w", err)
	}
	return nil
}

func (s *DRBToRemoveListEUTRAN) Decode(r *aper.AperReader) (err error) {

	// 1. Create a decoder function for the item type.
	decoder := func(r *aper.AperReader) (*DRBToRemoveItemEUTRAN, error) {
	...
	var decodedItems []DRBToRemoveItemEUTRAN // <--- FIX: Removed the '*'
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 0, Ub: 0}, false); err != nil {
		return fmt.Errorf("ReadSequenceOf for DRBToRemoveListEUTRAN failed: %w", err)
	}
