...
// Encode implements the aper.AperMarshaller interface for GNBCUUPE1SetupFailure.
func (msg *GNBCUUPE1SetupFailure) Encode(w io.Writer) error {
	ies, err := msg.toIes()
	if err != nil {
		return fmt.Errorf("could not convert GNBCUUPE1SetupFailure to IEs: %w", err)
	}

	return encodeMessage(w, E1apPduUnsuccessfulOutcome, ProcedureCode{Value: ProcedureCodeGNBCUUPE1Setup}, Criticality{Value: CriticalityIgnore}, ies)
}

// Decode implements the aper.AperUnmarshaller interface for GNBCUUPE1SetupFailure.
func (msg *GNBCUUPE1SetupFailure) Decode(buf []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	defer func() {
		if err != nil {
