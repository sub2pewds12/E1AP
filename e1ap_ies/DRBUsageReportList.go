...
func (s *DRBUsageReportList) Encode(w *per.Encoder) (err error) {

	c := per.SizeConstraints{Extensible: false, Min: int64Ptr(1), Max: int64Ptr(Maxnooftimeperiods)}
...
func (s *DRBUsageReportList) Decode(r *per.Decoder) (err error) {
	...
	s.Value = make([]DRBUsageReportItem, length)
