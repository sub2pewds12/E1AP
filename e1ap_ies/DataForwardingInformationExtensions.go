...
func (s *DataForwardingInformationExtensions) Encode(w *per.Encoder) error {
...
func (s *DataForwardingInformationExtensions) Decode(r *per.Decoder) error {
	c := per.SizeConstraints{Extensible: false, Min: int64Ptr(1), Max: int64Ptr(MaxProtocolExtensions)}
	...
	for _, ext := range extensions {
		...
		case ProtocolIEIDDataForwardingtoNGRANQoSFlowInformationList:
			...
			if err := s.DataForwardingtoNGRANQoSFlowInformationList.Decode(per.NewDecoder(ext.ValueBytes, per.APER)); err != nil {
