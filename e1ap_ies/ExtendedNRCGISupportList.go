...
func (s *ExtendedNRCGISupportList) Encode(w *aper.AperWriter) (err error) {
	...
	tmp := Sequence[aper.IE]{
		c:   aper.Constraint{Lb: 1, Ub: MaxnoofExtNRCGI},
		ext: false,
	}
	...
	if err = tmp.Encode(w); err != nil {
		err = fmt.Errorf("encode ExtendedNRCGISupportList failed: %w", err)
		return
	}
	return nil
}
...
func (s *ExtendedNRCGISupportList) Decode(r *aper.AperReader) (err error) {
	...
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 1, Ub: MaxnoofExtNRCGI}, false); err != nil {
