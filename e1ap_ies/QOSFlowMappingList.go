...
// QOSFlowMappingList is a generated LIST type.
type QOSFlowMappingList struct {
	Value []QOSFlowMappingItem
}

func (s *QOSFlowMappingList) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 1, Ub: MaxnoofQoSFlows}, false); err != nil {
		return fmt.Errorf("writeSequenceOf for QOSFlowMappingList failed: %w", err)
	}
	return nil
}

func (s *QOSFlowMappingList) Decode(r *aper.AperReader) (err error) {

	// 1. Create a decoder function for the item type.
	decoder := func(r *aper.AperReader) (*QOSFlowMappingItem, error) {
	...
	var decodedItems []QOSFlowMappingItem // <--- FIX: Removed the '*'
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 1, Ub: MaxnoofQoSFlows}, false); err != nil {
		return fmt.Errorf("readSequenceOf for QOSFlowMappingList failed: %w", err)
	}
