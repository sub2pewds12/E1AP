...
func (s *ProtocolExtensionField) Encode(w *aper.AperWriter) (err error) {
	...
	if err = s.ExtensionValue.Encode(w); err != nil {
		return fmt.Errorf("Encode ExtensionValue failed: %w", err)
	}
	return nil
}
...
func (s *ProtocolExtensionField) Decode(r *aper.AperReader) (err error) {
...

	{
		...
		if val, err = r.ReadOctetString(&aper.Constraint{Lb: 0, Ub: 0}, false); err != nil {
			return fmt.Errorf("Decode ExtensionValue failed: %w", err)
		}
		s.ExtensionValue = aper.OctetString(val)
	}

	if isExtensible {
		return fmt.Errorf("Extensions not yet implemented for ProtocolExtensionField")
	}
	return nil
}
