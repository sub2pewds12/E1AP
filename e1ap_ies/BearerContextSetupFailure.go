...
func (msg *BearerContextSetupFailure) Encode(w io.Writer) error {
	...
	return EncodeUnsuccessfulOutcome(w, ProcedureCodeBearerContextSetup, Criticality{Value: CriticalityIgnore}, ies)
}

// Decode implements the aper.AperUnmarshaller interface for BearerContextSetupFailure.
func (msg *BearerContextSetupFailure) Decode(buf []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("BearerContextSetupFailure: %w", err)
		}
	}()
