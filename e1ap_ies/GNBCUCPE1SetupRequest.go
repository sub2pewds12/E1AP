...
// toIes transforms the GNBCUCPE1SetupRequest struct into a slice of E1APMessageIEs.
...
// Encode implements the aper.AperMarshaller interface for GNBCUCPE1SetupRequest.
func (msg *GNBCUCPE1SetupRequest) Encode(w io.Writer) error {
	ies, err := msg.toIes()
	if err != nil {
		return fmt.Errorf("could not convert GNBCUCPE1SetupRequest to IEs: %w", err)
	}

	return encodeMessage(w, E1apPduInitiatingMessage, ProcedureCode{Value: ProcedureCodeGNBCUCPE1Setup}, Criticality{Value: CriticalityReject}, ies)
}

// Decode implements the aper.AperUnmarshaller interface for GNBCUCPE1SetupRequest.
func (msg *GNBCUCPE1SetupRequest) Decode(buf []byte) (diagList []CriticalityDiagnosticsIEItem, err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("decode GNBCUCPE1SetupRequest failed: %w", err)
		}
	}()

	r := aper.NewReader(bytes.NewReader(buf))

	decoder := GNBCUCPE1SetupRequestDecoder{
		msg:  msg,
...
type GNBCUCPE1SetupRequestDecoder struct {
	msg      *GNBCUCPE1SetupRequest
	diagList []CriticalityDiagnosticsIEItem
	list     map[ProtocolIEID]*E1APMessageIE
}

func (decoder *GNBCUCPE1SetupRequestDecoder) decodeIE(r *aper.AperReader) (msgIe *E1APMessageIE, err error) {
