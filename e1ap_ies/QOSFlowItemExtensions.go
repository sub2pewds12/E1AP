...
func (s *QOSFlowItemExtensions) Encode(w *per.Encoder) error {
...
func (s *QOSFlowItemExtensions) Decode(r *per.Decoder) error {
	c := per.SizeConstraints{Extensible: false, Min: int64Ptr(1), Max: int64Ptr(MaxProtocolExtensions)}
	...
	for _, ext := range extensions {
		...
		case ProtocolIEIDQoSFlowMappingIndication:
			...
			if err := s.QoSFlowMappingIndication.Decode(per.NewDecoder(ext.ValueBytes, per.APER)); err != nil {
