...
// GNBCUCPTNLAToRemoveList is a generated LIST type.
type GNBCUCPTNLAToRemoveList struct {
	Value []GNBCUCPTNLAToRemoveItem
}

func (s *GNBCUCPTNLAToRemoveList) Encode(w *aper.AperWriter) (err error) {
	...
	if err = aper.WriteSequenceOf(itemPointers, w, &aper.Constraint{Lb: 0, Ub: MaxnoofTNLAssociations}, false); err != nil {
		return fmt.Errorf("writeSequenceOf for GNBCUCPTNLAToRemoveList failed: %w", err)
	}
	return nil
}

func (s *GNBCUCPTNLAToRemoveList) Decode(r *aper.AperReader) (err error) {

	// 1. Create a decoder function for the item type.
	decoder := func(r *aper.AperReader) (*GNBCUCPTNLAToRemoveItem, error) {
	...
	var decodedItems []GNBCUCPTNLAToRemoveItem // <--- FIX: Removed the '*'
	if decodedItems, err = aper.ReadSequenceOf(decoder, r, &aper.Constraint{Lb: 0, Ub: MaxnoofTNLAssociations}, false); err != nil {
		return fmt.Errorf("readSequenceOf for GNBCUCPTNLAToRemoveList failed: %w", err)
	}
